<template>
  <div class=" container-fluid">
    <div class=" row">
          ...
          <div class="card-header filters-section">
            ...
            <div>
              <div class="filter-item">
                ...
                <MaterialInput id="startDate" type="date" v-model="bitacoraError.StartDate" @blur="formatStartDate" />
              </div>
            ...
            <div class="filter-item">
              ...
              <MaterialInput id="endDate" type="date" v-model="bitacoraError.EndDate" @blur="formatEndDate" />
            </div>

            <!-- Search Button -->
            <div class="filter-item button-container flex flex-row">
              <MaterialButton class="m-1" color="primary" size="md" variant="gradient"
                @click="fetchFilteredBitacoraError"
                  v-permiso="'bitacoradeerrores.Consultar'"
                >
                Buscar
              </MaterialButton>
      ...
      <!-- Data Table -->
      <DataTable title="Bitácora Error" description="Lista de Eventos" table-id="bitacora-table"
        :columns="bitacoraColumnsError" :rows="rowsBitacoraError" :searchable="true" :exportar="true"  permisoExportar="bitacoradeerrores.Exportar"  />
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const { rowsBitacoraError, bitacoraColumnsError, users, bitacoraError } = storeToRefs(bitacoraStore);
    ...
    const fetchFilteredBitacoraError = async () => {
      await bitacoraStore.fetchBitacoraError();
    };

    const cleanBitacoraError = () => {
      bitacoraStore.cleanBitacoraError();
    };
